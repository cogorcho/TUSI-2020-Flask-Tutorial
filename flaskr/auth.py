...
Then the blueprint is registered with the application when it is available in 
...
with a form for them to fill out. 
When they submit the form, it will validate their input and either show the form 
again with an error message or create the new user and go to the login page.
"""
...
"""
Login: This view follows the same pattern as the register view above
"""
...
Flask securely signs the data so that it can't be tampered with.11G
"""
...
load_logged_in_user checks if a user id is stored in the session and gets that 
user's data from the database, storing it on g.user, which lasts for the length 
of the request. 
